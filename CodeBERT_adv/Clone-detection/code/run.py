...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512, pool=None):
        ...
        try:
            ...
            self.examples = [get_example(example) for example in data]
...
def load_and_cache_examples(args, tokenizer, evaluate=False, test=False, pool=None):
    ...
    if args.use_enhance and not test and not evaluate:
...
def main():
    ...
    if args.do_train:
        ...
        if args.use_enhance:
            output_dir = 'checkpoint-best-f1/codebert_model.bin'
